...
| `ping_pong` | no | If `true` and `sequence` is omitted: play **1→N** then **N−1→2**. Doubles loop time and replays hold frames on the way back. |
| `skip_range` | no | Two integers `[from, to]` (1-based inclusive): skip that index range. Example: `[31, 130]` on look = idle + look up (1–30) + return (131–149). |
